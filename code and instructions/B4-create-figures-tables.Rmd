...
### IMPORT: Census features before subsetting data
census_clean <- import('data/processed/preprocessing/nevi_tract_features_beforesubset.csv')
...
### 3 subplots together in one
  # Changed pad_x and pad_y to 1.5 --> 0.25
fig1_all <- ggarrange(fig1_part1, fig1_part2, 
          ggarrange(fig1_hlth, ggplot() + theme_minimal(), nrow = 2, labels = "E", font.label = list(size = 25)) + theme(plot.margin = margin(t = 0, r = 0, b = 1, l = 0, "cm")), ncol = 3)
...
fig1_all
ggsave('figures and tables/figures/fig1_all.png', width = 20, height = 8) 
ggsave('figures and tables/figures/eps/fig1_all.eps', width = 20, height = 8) 
...
### 2.3. Figure 3 - NEVI Cluster Map + Cluster Profile
...
# Plotting
fig3_part1_prelim <- ggplot(fig3_part1_df, aes(x = nevi_cat, y = mean, fill = nevi_cluster_label, pattern = nevi_cat)) +
...
fig3_part1_plot_build <- ggplot_build(fig3_part1_prelim)
for(i in 1:6){
  fig3_part1_plot_build[["layout"]][["panel_params"]][[i]][["r.range"]][2] <- 0.45
}
fig3_part1 <- ggplot_gtable(fig3_part1_plot_build)


## CREATE: PART 2 - CLUSTER MAP
gen_fig3_part2 <- function(dfin, dfexcl, dfborough, fill_var, fill_color, fill_label, fill_increment){
  plot_choro <- ggplot() + 
    geom_sf(data = dfin, aes(fill = !!sym(fill_var)), color = NA) + 
...
fig3_part2 <- gen_fig3_part2(dfin = tract_final_spatial, dfexcl = tract_exclude_spatial, dfborough = borough_nyc_spatial, fill_var = "nevi_cluster", fill_color = colors_colorblindsafe, fill_label = "Neighborhood Environmental Vulnerability Index Cluster") 

## COMBINED PLOT
fig3b <- ggarrange(fig3_part1, ggarrange(fig3_part2, ggplot() + theme_void(), ncol = 2), nrow = 2) + theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 0, "cm"))
fig3b
# EXPORT
ggsave('figures and tables/figures/fig3_part.png', width = 7, height = 9) # pixels: 1200 x 900 width = 13, height = 12
ggsave('figures and tables/figures/eps/fig3_part.eps', width = 7, height = 9)
# View average NEVI domains scores by cluster for text
fig3b_part1_df %>% distinct(nevi_cluster, mean_nevi, iqr_nevi)

```

### 2.4. Figure 4 - NEVI Cluster Subdomains Heatmap
...
##### CREATE: heatmap 
ggplot(data = fig4_df, aes(x = cluster_label, y = subdomain_label, fill = median_scaled)) +
  geom_tile() + 
...
## EXPORT
ggsave('figures and tables/figures/fig4.png', width = 10, height = 13)
ggsave('figures and tables/figures/eps/fig4.eps', width = 10, height = 13)
```

### 2.3. Figure 5 - NEVI and Subdomain Median Scores by Race and Ethnicity
...
census_raceeth <- census_clean %>% 
  dplyr::rename(Tract_FIPS = tract) %>% 
  dplyr::select_at(c('Tract_FIPS',vars_raceeth))
tract_raceeth_prelim <- tract_final %>% 
  dplyr::left_join(census_raceeth, by = 'Tract_FIPS') 
# Calculate median by Race/ethnicity and create indicator variable
median_raceeth <- tract_raceeth_prelim %>% 
  dplyr::summarize(med_aian_nonhisp_prop = median(aian_nonhisp_prop), # median of 0
                   med_asian_nonhisp_prop = median(asian_nonhisp_prop),
                   med_black_nonhisp_prop = median(black_nonhisp_prop),
                   med_hisp_prop = median(hisp_prop),
                   med_nhpi_nonhisp_prop = median(nhpi_nonhisp_prop), # median of 0
                   med_white_nonhisp_prop = median(white_nonhisp_prop),
                   med_race_mult_nonhisp_prop = median(race_mult_nonhisp_prop))
tract_raceeth <- tract_raceeth_prelim %>% 
  dplyr::mutate(nb_aian  = ifelse(aian_nonhisp_prop      > median_raceeth$med_aian_nonhisp_prop, 1, 0),
                nb_asian = ifelse(asian_nonhisp_prop     > median_raceeth$med_asian_nonhisp_prop, 1, 0),
                nb_black = ifelse(black_nonhisp_prop     > median_raceeth$med_black_nonhisp_prop, 1, 0),
                nb_hisp  = ifelse(hisp_prop              > median_raceeth$med_hisp_prop, 1, 0),
                nb_nhpi  = ifelse(nhpi_nonhisp_prop      > median_raceeth$med_nhpi_nonhisp_prop, 1, 0),
                nb_white = ifelse(white_nonhisp_prop     > median_raceeth$med_white_nonhisp_prop, 1, 0),
                nb_mult  = ifelse(race_mult_nonhisp_prop > median_raceeth$med_race_mult_nonhisp_prop, 1, 0))
## Calculate median NEVI/domain scores by racial/ethnic group
# Function to calculate
calcmed_nb_race <- function(df, col){
  df %>% 
    dplyr::group_by(!!sym(col)) %>% 
    dplyr::summarize(nb_race = col,
                     med_nevi = median(nevi),
                     med_demo = median(score_demo),
                     med_economic = median(score_economic),
                     med_residential = median(score_residential),
                     ned_healthstatus = median(score_healthstatus)) %>% 
    tidyr::pivot_longer(cols = c(med_nevi, med_demo, med_economic, med_residential, ned_healthstatus), names_to = 'domain', values_to = 'median') %>% 
    dplyr::filter(!!sym(col) == 1) %>% 
    dplyr::select(-!!sym(col)) 
}
fig5_df <- bind_rows(
    calcmed_nb_race(tract_raceeth, 'nb_aian'),
    calcmed_nb_race(tract_raceeth, 'nb_asian'),
    calcmed_nb_race(tract_raceeth, 'nb_black'),
    calcmed_nb_race(tract_raceeth, 'nb_hisp'),
    calcmed_nb_race(tract_raceeth, 'nb_nhpi'),
    calcmed_nb_race(tract_raceeth, 'nb_white'),
    calcmed_nb_race(tract_raceeth, 'nb_mult')) %>% 
  dplyr::mutate(
      nb_race_label = dplyr::case_when(
        nb_race == 'nb_aian' ~ 'American\nIndian/\nAlaskan\nNative',
        nb_race == 'nb_asian' ~ 'Asian',
        nb_race == 'nb_black' ~ 'Black',
        nb_race == 'nb_hisp' ~ 'Hispanic',
        nb_race == 'nb_mult' ~ 'Multiple\nRace',
        nb_race == 'nb_nhpi' ~ 'Native\nHawaiian/\nPacific\nIslander',
        nb_race == 'nb_white' ~ 'White'),
      domain = dplyr::case_when(
        domain == 'med_nevi' ~ 'Overall (NEVI)',
        domain == 'med_demo' ~ 'Demographic',
        domain == 'med_economic' ~ 'Economic Indicators',
        domain == 'med_residential' ~ 'Residential',
        domain == 'ned_healthstatus' ~ 'Health Status') %>% factor(levels = c('Overall (NEVI)','Demographic','Economic Indicators','Residential','Health Status')),
      domain_group = ifelse(domain == 'Overall (NEVI)', 'Overall', 'Domain') %>% factor(levels = c('Overall', 'Domain')),
      median_label = median %>% round(2) %>% format(2))

##### CREATE: heatmap 
fig5 <- ggplot(data = fig5_df, aes(x = nb_race_label, y = domain, fill = median)) +
  geom_tile() + 
  geom_text(aes(label = median_label), size = 7) +
  facet_grid(domain_group ~ ., scales = 'free_y', space = 'free_y') +
  scale_y_discrete(limits = rev) +
  scale_fill_gradient(low = 'white', high = 'red') +
  labs(x = 'High Racial and Ethnic Composition\n(Proportion > Citywide Median Proportion)', y = 'Vulnerability', fill = "Median Score") +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', color = 'white'),
        strip.text.y = element_blank(),
        axis.title.x = element_text(size = 20, margin = margin(t = 10)), axis.text.x = element_text(size = 15),
        axis.title.y = element_text(size = 20, margin = margin(r = 10)), axis.text.y = element_text(size = 15), 
        strip.text = element_text(size = 16),
        legend.title = element_text(size = 16), legend.text = element_text(size = 14),
        legend.position = 'top', legend.key.width = unit(1.5, 'cm'), legend.spacing.x = unit(1, 'cm'))
...
### EXPORT: Plots
fig5
ggsave('figures and tables/figures/fig5.png', width = 10, height = 7)
ggsave('figures and tables/figures/eps/fig5.eps', device = cairo_ps, width = 10, height = 7)
...
### 2.3. S Figure 2 A/B - Scatter plots NEVI vs. NDI/SVI
...
gen_figs2_scatterplot <- function(datain, var, var_label, var_breaks){
...
### CREATE: Plots
figs2a_ndi <- gen_figs2_scatterplot(datain = figs2_df, var = "ndi", var_label = "Neighborhood Deprivation Index", var_breaks = 2) # NDI
figs2b_svi <- gen_figs2_scatterplot(datain = figs2_df, var = "svi", var_label = "Social Vulnerability Index", var_breaks = 0.2) # SVI
### EXPORT: Plots
figs2a_ndi
ggsave('figures and tables/figures/figs2a.png', width = 15, height = 12)
ggsave('figures and tables/figures/eps/figs2a.eps', device = cairo_ps, width = 15, height = 12)
figs2b_svi
ggsave('figures and tables/figures/figs2b.png', width = 15, height = 12)
ggsave('figures and tables/figures/eps/figs2b.eps', device = cairo_ps, width = 15, height = 12)
```
