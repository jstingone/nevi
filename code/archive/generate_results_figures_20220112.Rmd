...
tract_final %>% 
  ...
  dplyr::left_join(gen_stab5(tract_final, 'score_demo')) %>% dplyr::rename(demo_median_iqr = median_iqr) %>% 
  dplyr::left_join(gen_stab5(tract_final, 'score_economic')) %>% dplyr::rename(economic_median_iqr = median_iqr) %>% 
  dplyr::left_join(gen_stab5(tract_final, 'score_residential')) %>% dplyr::rename(residential_median_iqr = median_iqr) %>% 
  dplyr::left_join(gen_stab5(tract_final, 'score_healthstatus')) %>% dplyr::rename(healthstatus_median_iqr = median_iqr) 
# text_summary %>% View()
export(stab5, 'tabs/stab5.xlsx')
```
