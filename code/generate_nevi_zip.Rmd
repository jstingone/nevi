...
## A1. COMPARISON TO ZIP FROM SENSITIVITY ANALYSIS
```{r}
# import
nevi_zip <- readRDS(file = "data/processed/nevi_final_zip.rds")
nevi_zip_sa <- readRDS(file = "data/processed/sensitivity analysis/nevi zip code rescale features/nevi_zip_final_sa.rds") %>% 
  dplyr::as_tibble() %>% 
  dplyr::transmute(zip = ZIP,
                   nevi_sa = nevi,
                   score_demo_sa = score_demo,
                   score_economic_sa = score_economic,
                   score_residential_sa = score_residential,
                   score_healthstatus_sa = score_healthstatus)
# merge
nevi_zip_comb <- nevi_zip_sa %>% 
  dplyr::left_join(nevi_zip, by = 'zip') %>% 
  dplyr::mutate(diff_nevi = nevi - nevi_sa,
                diff_score_demo = score_demo - score_demo_sa,
                diff_score_economic = score_economic - score_economic_sa,
                diff_score_residential = score_residential - score_residential_sa,
                diff_score_healthstatus = score_healthstatus - score_healthstatus_sa)

# zip in SA but not in main crosswalk calculation 
  # identify
nevi_zip_comb %>% filter(is.na(nevi)) # 11003, 11005
  # prep spatial
library(sf)
zip_spatial <- tigris::zctas(state = 'NY', year = 2010, cb = FALSE) %>% 
  dplyr::rename(zip = ZCTA5CE10) %>% 
  dplyr::inner_join(nevi_zip_comb, by = 'zip') %>% 
  dplyr::mutate(flag_sa_notmain = ifelse(zip %in% c('11003','11005'), 1, 0))
  # map
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = flag_sa_notmain)) + 
  theme_minimal()

# map differences
  # nevi
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = diff_nevi)) + 
  theme_minimal()
  # demo
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = diff_score_demo)) + 
  theme_minimal()
  # economic
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = diff_score_economic)) + 
  theme_minimal()
  # residential
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = diff_score_residential)) + 
  theme_minimal()
  # healthstatus
ggplot() +
  geom_sf(data = zip_spatial, aes(fill = diff_score_healthstatus)) + 
  theme_minimal()
```
