...
library(tidycensus)
census_api_key("855da703f5099abb753fe0b3f3fd144f8afe73dc") # Please update with your own key.
options(tigris_use_cache = TRUE)
```

## Import Data
Download the 2020 Centers for Disease Control and Disease Prevention (CDC) PLACES data at the Census tract level at the following link: https://chronicdata.cdc.gov/500-Cities-Places/PLACES-Local-Data-for-Better-Health-Census-Tract-D/cwsq-ngmh  
The file is also available in the repository. 

```{r}
# 500 Cities Project
PLACES_orig <- rio::import('data/raw/PLACES__Census_Tract_Data__GIS_Friendly_Format___2020_release.csv')
# Exclusions List
...
header_toxpi <- read.csv("data/processed/toxpi headers/toxpi_header.csv", header = F, colClasses = rep("character",62))
# NYC Counties
...
```{r}
### CLEAN: Census + Merge with rest
  # NOTE: Housing (B25035_001E/age_structure_2019): See value of 18 for Tract = 36081001900 (pop = 4358). Plugging in missing for those values now
gen_census_features <- function(indf, var_area){
  outdf <- indf %>% 
    ...
    dplyr::transmute(GEOID = as.character(GEOID),
                     ...
                     misc_nonwhite_prop = (B03002_001E - B03002_003E)/B03002_001E, ### to double check- maybe grab other variables, just subtracting NLW pop from whole pop here
...
### Merge Data from CDC 500 Cities project and U.S. Census and apply Exclusions
```{r}
...
## Combine Data with ToxPi header to use in the ToxPi GUI
```{r toxpi_header}
...
## EXPORT: Data
```{r export_features, include=FALSE}
export(toxpi_export, "data/processed/nvi_features.csv", col.names = FALSE)
```


## Using the ToxPi GUI
1. Download ToxPi Graphical User Interface (GUI) at this link. We used version 2.3 in our original version of the NVI (August 2019 update): https://toxpi.org/
2. Follow the steps in the NVI ToxPi GUI tutorial (nvi_toxpi_gui_tutorial.docx) to generate the NVI before running the rest of the R code.


## IMPORT: Data from ToxPi GUI
```{r}
tract_toxpi_pre <- import('data/processed/toxpi exports/nvi_toxpi_results.csv')
```

## CLEAN: Clean data from ToxPi GUI
```{r}
...
# MERGE AND CLEAN DATA: NVI features + NVI (from ToxPi GUI) + Clusters
