---
title: "Developing a Neighborhood Vulnerability Index in New York City, 2019"
author: 
- "Stephen P. Uong" 
...
In this document, we have summarized our process that we followed to generate the NVI. 
...
- List of Census Tracts to be excluded because of 1) a population of less than 20 or 2) at least 1 missing feature for the NVI or Neighorhood Deprivation Index (NDI), one of the indices to which the NVI was compared. 
- Header needed to import the spreadsheet into the ToxPi Graphical User interface used to generate the NVI
...
We merged the data from the CDC 500 Cities project and the U.S. Census before appylying our exclusions, census tracts that had 1) a population of less than 20 and 2) at least 1 missing feature for the NVI or Neighorhood Deprivation Index (NDI), one of the indices to which the NVI was compared. 
```{r, include = FALSE}
...
# Merge and apply exclusions: 1) < 20 pop, 2) at least 1 missing feature for NVI or NDI
...
### 5. Exporting the Features Used in the NVI
We exported our data into a spreadsheet with the NVI features and the header needed to import the spreadsheet into the ToxPi GUI.
```{r, export_features}
export(tract_exclusions, "data/processed/nvi_features.csv", col.names = TRUE)
export(toxpi_export, "data/processed/nvi_features_toxpi.csv", col.names = FALSE)
  # For sensitivity analyses of race/ethnicity
export(toxpi_export_raceeth_olap, "data/processed/nvi_features_toxpi_sa_raceeth_olap.csv", col.names = FALSE)
export(toxpi_export_raceeth_no_olap, "data/processed/nvi_features_toxpi_sa_raceeth_no_olap.csv", col.names = FALSE)
export(toxpi_export_raceeth_hispsub, "data/processed/nvi_features_toxpi_sa_raceeth_hispsub.csv", col.names = FALSE)
```


### 6. Generating the NVI using the ToxPi GUI
To generate the NVI in the ToxPi GUI, we followed the following steps:
1. Downloaded ToxPi Graphical User Interface (GUI) at this link. We used version 2.3 in our original version of the NVI (August 2019 update): https://toxpi.org/
2. Followed the steps in the NVI ToxPi GUI tutorial (nvi_toxpi_gui_tutorial.docx) to generate the NVI before running the rest of the R code.
...
### 8. Creating the NVI Clusters 
We created clusters from the slices/subdomains for the NVI.
```{r, create_nvi_clusters}
...
tract_nvi_cluster <- tract_toxpi_pre %>% 
  dplyr::mutate(SID = Source %>% as.character() %>% trimws(),
                ...
                nvi_cluster = case_when(
                  nvi_cluster_orig == "1" ~ "6",
                  nvi_cluster_orig == "2" ~ "5",
                  nvi_cluster_orig == "3" ~ "4",
                  nvi_cluster_orig == "4" ~ "3",
                  nvi_cluster_orig == "5" ~ "2",
                  nvi_cluster_orig == "6" ~ "1"
                ) %>% as.factor()) %>% 
  dplyr::select(SID, nvi_cluster)
```
...
We renamed the subdomain slices and manually calculated domain-specific scores for the NVI.
...
### 9. Merging the NVI with the NVI features and clusters
We merged the following data: NVI, NVI features, and NVI clusters.
```{r, merge_index}
tract_final <- tract_exclusions %>% # NVI features
  dplyr::left_join(tract_toxpi, by = "SID") %>%  # NVI (from the ToxPi GUI)
  dplyr::left_join(tract_nvi_cluster, by = "SID") %>%  # NVI clusters
  dplyr::rename(Tract_FIPS = SID) %>% 
  dplyr::left_join(tract_nyc_county, by = "Tract_FIPS") %>% 
  dplyr::relocate(county, nvi, nvi_cluster, score_demo, score_economic, score_residential, score_healthstatus, .after = Tract_FIPS) %>% 
  dplyr::select(-c(row, CASRN, Name)) 
```


### 10. Exporting the final data with the NVI, features, and clusters
After completing all importing and data cleaning steps, we exported our final dataset, which included the NVI, NVI features, and NVI clusters.
```{r, export_final_data}
export(tract_final, paste0("data/processed/nvi_final.csv"))
saveRDS(tract_final, file = "data/processed/nvi_final.rds")
