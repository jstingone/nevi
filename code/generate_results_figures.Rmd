---
title: "Describing the NEVI and Comparison the the NEVI to the NDI/SVI: Results and Figures"
author: 
- "Stephen P. Uong" 
...
tract_toxpi_raceeth_olap_df <- clean_toxpi_sa(tract_toxpi_pre_raceeth_olap, tract_nyc_borough) %>% 
  dplyr::mutate(score_demo = (score_demo_age + score_demo_femaleled + score_demo_raceeth + score_demo_immigration + score_demo_disability + score_demo_singleparent + score_demo_mobility + score_demo_livealone)/8) 
  # Race/eth + Hisp
tract_toxpi_raceeth_hsub_df <- clean_toxpi_sa(tract_toxpi_pre_raceeth_hsub, tract_nyc_borough) %>% 
...
### BOXPLOT
# PREP data
figs3_df <- rbind(tract_final %>% dplyr::select(nevi, score_demo, borough) %>% dplyr::mutate(index_type = "Main"),
               tract_toxpi_raceeth_no_olap_df %>% dplyr::select(nevi, score_demo, borough) %>% dplyr::mutate(index_type = "Non-Overlapping Race and Ethnicity\nCategories"),
               tract_toxpi_raceeth_olap_df %>% dplyr::select(nevi, score_demo, borough) %>% dplyr::mutate(index_type = "Overlapping Race and Ethnicity\nCategories"),
               tract_toxpi_raceeth_hsub_df %>% dplyr::select(nevi, score_demo, borough) %>% dplyr::mutate(index_type = "Hispanic Separate")) %>% 
  dplyr::mutate(all = "All Boroughs", index_type = factor(index_type, levels = c("Main",
                                                           "Non-Overlapping Race and Ethnicity\nCategories",
                                                           "Overlapping Race and Ethnicity\nCategories",
                                                           "Hispanic Separate")))
figs3_sub_df <- rbind(tract_toxpi_raceeth_no_olap_df %>% dplyr::select(nevi, score_demo_raceeth, borough) %>% dplyr::mutate(index_type = "Non-Overlapping Race and Ethnicity\nCategories"),
                      tract_toxpi_raceeth_olap_df %>% dplyr::select(nevi, score_demo_raceeth, borough) %>% dplyr::mutate(index_type = "Overlapping Race and Ethnicity\nCategories\nRace and Ethnicity Subdomain"),
                      tract_toxpi_raceeth_hsub_df %>% dplyr::select(nevi, score_demo_raceeth, borough) %>% dplyr::mutate(index_type = "Hispanic Separate\nRace Subdomain"),
                      tract_toxpi_raceeth_hsub_df %>% dplyr::select(nevi, score_demo_raceeth_hisp, borough) %>% dplyr::rename(score_demo_raceeth = score_demo_raceeth_hisp) %>% dplyr::mutate(index_type = "Hispanic Separate\nHispanic Subdomain")) %>% 
  dplyr::mutate(all = "All Boroughs", index_type = factor(index_type, levels = c("Non-Overlapping Race and Ethnicity\nCategories",
                                                           "Overlapping Race and Ethnicity\nCategories\nRace and Ethnicity Subdomain",
                                                           "Hispanic Separate\nRace Subdomain",
                                                           "Hispanic Separate\nHispanic Subdomain")))
...
figs3c <- ggplot(figs3_sub_df, aes(x = score_demo_raceeth, fill = index_type)) +
  ...
  scale_y_discrete(limits = rev) +
  geom_hline(yintercept = 5.5, color = "gray20", linetype = "dashed") +
  labs(title = "Subdomain: Race and Ethnicity Score", fill = "Race and Ethnicity Subdomain:\nRace and Ethnicity Categorization") +
...
ggsave('figs/figs3.png', plot = figs3_export, width = 15, height = 20)
...
w_raceeth <- sapply(strsplit(w,split="/"),function(x) {y <- ifelse(length(x)==2,x[2],1);as.numeric(x[1])/as.numeric(y)})
  # Generate cluster
hc_raceeth <- hclust(dist(tract_toxpi_pre_raceeth_no_olap[,-c(1:5)]*rep(w,each=nrow(tract_toxpi_pre_raceeth_no_olap))), method="complete")
nevi_cluster_result_raceeth <- cutree(hc, k = 6) # Cut into 6 clusters using HClusts 
