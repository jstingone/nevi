...
author: "Stephen P. Uong, Jiayi Zhou, Jeanette A. Stingone"
date: "10/28/2021"
output: 
  html_document: 
    toc: true
...
## IMPORT: Cleaned/toxpi data
```{r}
### IMPORT: TOXPI
tract_exclusions <- readRDS("data/processed/tract_features_20210826.rds")
tract_toxpi_pre <- import('data/processed/toxpi exports/toxpi_tract_20211013_results.csv')
...
### IMPORT: NDI, SVI, MH SVI
ndi <- import("data/from others/20210721 helen scores/ndi.csv")
svi <- import("data/from others/20210721 helen scores/svi.csv")
...
clean_toxpi <- function(datain){
  dataout <- datain %>% 
    dplyr::rename(SID = Source, nvi = `ToxPi Score`) %>% 
    ...
    # Checked: Calculated NVI score is same as one calculated in toxpi (just rounding differences)
...
fviz_gap_stat(gap_stat_10) # choose 6 clusters
  # Get slice weights
...
hc <- hclust(dist(tract_toxpi_pre[,-c(1:5)]*rep(w,each=nrow(tract_toxpi_pre))), method="complete")
  # Plot: plot(hc,cex = 0.6,hang=-1)
nvi_cluster_result <- cutree(hc, k = 6) # Cut into 6 clusters using HClusts 
tract_nvi_cluster <- tract_toxpi_pre %>% 
  dplyr::mutate(SID = Source %>% as.character() %>% trimws(),
                nvi_cluster_orig = nvi_cluster_result %>% as.factor(),
                nvi_cluster = case_when(
                  nvi_cluster_orig == "1" ~ "4",
                  nvi_cluster_orig == "2" ~ "6",
                  nvi_cluster_orig == "3" ~ "5",
                  nvi_cluster_orig == "4" ~ "1",
                  nvi_cluster_orig == "5" ~ "3",
                  nvi_cluster_orig == "6" ~ "2"
                ) %>% as.factor()) %>% 
  dplyr::select(SID, nvi_cluster_orig, nvi_cluster)

# MERGE: With original features and nycgeo tract boundaries
  # NOTE: Discrepancy in tracts nycgeo vs. Census: nycgeo has 2166 tracts vs our original 2167 from US Census for some reason. 2165 overlapping
  # ALL CLEAR-- There is NO issue when we just look at our included tracts
tract_final <- tract_exclusions %>% 
  dplyr::left_join(tract_toxpi, by = "SID") %>% 
  #dplyr::mutate(nvi_cluster = nvi_cluster %>% as.factor()) %>% # NVI clusters
  dplyr::left_join(tract_nvi_cluster, by = "SID") %>% 
  dplyr::rename(Tract_FIPS = SID) %>% 
  dplyr::left_join(tract_nyc_county, by = "Tract_FIPS") %>% 
  dplyr::relocate(county, nvi, score_demo, score_economic, score_residential, score_healthstatus, .after = Tract_FIPS) %>% 
  dplyr::select(-c(row, CASRN, Name)) 
# Spatial: crs = "+init=epsg:2263"
  # Included
tract_final_spatial <- tract_nyc_spatial %>% 
  dplyr::inner_join(tract_final, by = "Tract_FIPS") # Checked: did not exclude any tracts, 8/11
  # Excluded
tract_exclude_spatial <- tract_nyc_spatial %>% 
  dplyr::anti_join(tract_final, by = "Tract_FIPS") 

# CHECK: All subdomains range from 0 to 1
tract_toxpi %>% summary()
...
tract_toxpi_pre_raceeth_none <- import('data/processed/toxpi exports/toxpi_tract_20210826_raceeth_none_results.csv')
tract_toxpi_pre_raceeth_olap <- import('data/processed/toxpi exports/toxpi_tract_20210826_raceeth_olap_results.csv')
tract_toxpi_pre_raceeth_hsub <- import('data/processed/toxpi exports/toxpi_tract_20210826_raceeth_hsub_results.csv')
...
clean_toxpi_sa <- function(datain, datacounty){
  dataout <- datain %>% 
    ...
    dplyr::rename_at(vars(starts_with('Health StatusPreventive')), funs(paste0('score_healthstatus_preventive'))) %>% 
    dplyr::rename_at(vars(starts_with('Health StatusLackInsurance')), funs(paste0('score_healthstatus_lackinsurance'))) %>% 
    dplyr::left_join(datacounty %>% dplyr::rename(SID = Tract_FIPS), by = "SID")
  return(dataout)
}
tract_toxpi_raceeth_none_df %>% head(n=1)
# DATA CLEANING
  # Race/eth: none
tract_toxpi_raceeth_none_df <- clean_toxpi_sa(tract_toxpi_pre_raceeth_none, tract_nyc_county) %>% 
...
tract_toxpi_raceeth_none_df_corr <- tract_toxpi_raceeth_none_df %>% 
  dplyr::transmute(Tract_FIPS = SID,
                   NVI_raceeth_none = nvi,
                   DOMAIN_demo_raceeth_none = score_demo)
  # Race/eth: overlapping
tract_toxpi_raceeth_olap_df <- clean_toxpi_sa(tract_toxpi_pre_raceeth_olap, tract_nyc_county) %>% 
  dplyr::mutate(score_demo = (score_demo_age + score_demo_femaleled + score_demo_raceeth + score_demo_immigration + score_demo_disability + score_demo_singleparent + score_demo_mobility + score_demo_livealone)/8) 
tract_toxpi_raceeth_olap_df_corr <- tract_toxpi_raceeth_olap_df %>% 
  dplyr::transmute(Tract_FIPS = SID,
                   NVI_raceeth_olap = nvi,
                   DOMAIN_demo_raceeth_olap = score_demo,
                   SUBDOMAIN_demo_raceeth_olap_sub = score_demo_raceeth)
  # Race/eth + Hisp
...
tract_toxpi_raceeth_hsub_df_corr <- tract_toxpi_raceeth_hsub_df %>% 
  dplyr::transmute(Tract_FIPS = SID,
                   NVI_raceeth_hsub = nvi,
                   DOMAIN_demo_raceeth_hsub = score_demo,
                   SUBDOMAIN_demo_raceeth_hsub_sub_race = score_demo_raceeth,
                   SUBDOMAIN_demo_raceeth_hsub_sub_hisp = score_demo_raceeth_hisp)
...
ggsave(paste0('comms/figs/figs3_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 15, height = 20)
  


### CORRELATIONS - EXTRA
tract_exclusions_sa %>% head()
tract_exclusions_sa %>% names()
tract_exclusions_sa_raceeth <- tract_exclusions_sa %>% 
  dplyr::rename(Tract_FIPS = SID) %>% 
  dplyr::select(Tract_FIPS, white_prop,black_prop,asian_prop,aian_prop,nhpi_prop,race_other_prop,race_mult_prop,aian_nhpi_mult_other_prop,hisp_prop,white_nonhisp_prop,black_nonhisp_prop,asian_nonhisp_prop,aian_nonhisp_prop,nhpi_nonhisp_prop,race_other_nonhisp_prop,race_mult_nonhisp_prop,aian_nhpi_mult_other_nonhisp_prop)
tract_sa_comb <- tract_final %>% 
  dplyr::transmute(Tract_FIPS = Tract_FIPS, NVI = nvi, DOMAIN_demo = score_demo, SUBDOMAIN_demo_raceeth = score_demo_raceeth) %>% 
  dplyr::left_join(tract_toxpi_raceeth_none_df_corr, by = "Tract_FIPS") %>% 
  dplyr::left_join(tract_toxpi_raceeth_olap_df_corr, by = "Tract_FIPS") %>% 
  dplyr::left_join(tract_toxpi_raceeth_hsub_df_corr, by = "Tract_FIPS") %>% 
  dplyr::left_join(tract_exclusions_sa_raceeth, by = "Tract_FIPS") %>% 
  dplyr::select(-Tract_FIPS) %>% 
  dplyr::relocate(NVI, NVI_raceeth_none, NVI_raceeth_olap, NVI_raceeth_hsub,
                  DOMAIN_demo, DOMAIN_demo_raceeth_none, DOMAIN_demo_raceeth_olap, DOMAIN_demo_raceeth_hsub)
library(corrplot)
cor_tract_sa_comb <- cor(tract_sa_comb, method = "spearman") # use = "complete.obs", 
cortest_tract_sa_comb <- cor.mtest(tract_sa_comb)

corrplot(cor_tract_sa_comb, p.mat = cortest_tract_sa_comb$p, sig.level = 0.05, method = "circle")
corrplot(cor_tract_sa_comb, p.mat = cortest_tract_sa_comb$p, sig.level = 0.05, method = "number") # Really hard to see numbers
```
...
## EXPORT
fig1_part1
ggsave(paste0('comms/figs/fig1_part1_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 10, height = 15) # pixels: 1200 x 900, width = 15, height = 12
fig1_part2
ggsave(paste0('comms/figs/fig1_part2_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 10, height = 15) 
fig1_part3
ggsave(paste0('comms/figs/fig1_part3_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 10, height = 8) 
...
fig2
ggsave(paste0('comms/figs/fig2_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 13, height = 6) # pixels: 1200 x 900 width = 13, height = 12
...
### EXPORT: Plots
fig3_ndi
ggsave(paste0('comms/figs/fig3_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 15, height = 12)
figs2_svi
ggsave(paste0('comms/figs/figs2_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 15, height = 12)
...
fig4b
# EXPORT
ggsave(paste0('comms/figs/fig4_partb_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 7, height = 9) # pixels: 1200 x 900 width = 13, height = 12
...
ggsave(paste0('comms/figs/figs1_', Sys.Date() %>% format("%Y%m%d"), ".png"), width = 15, height = 16)



## Alt
ggplot(boxplot_df %>% filter(part == 'A'), aes(y = index_type, x = index, fill = county)) +
  geom_boxplot() +
  facet_grid(county~.) 
